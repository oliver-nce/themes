"""Shared CSS publish plumbing for the Web and Desk theme writers.

Extracted from css_writer.py / desk_css_writer.py (Stage 2 of the refactor) to
remove the duplicated file-write + hash-sidecar logic. Cache busting and the
Site Theme Config css_hash update stay in the callers because their ordering and
DB side-effects intentionally differ between Web and Desk.

write_published_css writes <css_filename> and its .hash sidecar under
public/css and returns the 8-char SHA-1 hash used by hooks.py for cache-busting.
"""
